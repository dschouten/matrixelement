...
C     
C     PROCESS: G G > Y QTD=2 WEIGHTED=2
C     *   DECAY: Y > W+ W- QTD=2 WEIGHTED=2
C     *     DECAY: W+ > E+ VE QTD=2 WEIGHTED=2
C     *     DECAY: W- > E- VE~ QTD=2 WEIGHTED=2
C     
      IMPLICIT NONE
      INTEGER RESET
...
C     ----------
      ...
      DO IHEL=1,NCOMB
        IF ( GOODHEL(IHEL) .OR. NTRY .LT. 10 ) THEN 
           IF( IHEL.EQ.ISELECTEDHEL .OR. ISELECTEDHEL.LE.0 ) THEN ! dschoute - conditional to skip all non-selected
              T=GGYWW(P ,NHEL(1,IHEL),JC(1))
              ANS=ANS+T
              MECOMB(IHEL)=T/DBLE(IDEN) ! dschoute - keep track of helicity combinations
              IF (T .NE. 0D0 .AND. .NOT. GOODHEL(IHEL)) THEN
                 PRINT *,'ADDED GOOD HELICITY ',IHEL,T,
     +                NHEL(1,IHEL),NHEL(2,IHEL)
                 GOODHEL(IHEL)=.TRUE.
              ENDIF
           ENDIF
        ENDIF
      ...
      REAL*8 FUNCTION GGYWW(P,NHEL,IC)
C     
C     GENERATED BY MADGRAPH 5 V. 1.4.5, 2012-04-20
C     BY THE MADGRAPH DEVELOPMENT TEAM
C     PLEASE VISIT US AT HTTPS://LAUNCHPAD.NET/MADGRAPH5
...
C     ----------
      ...
      CALL FFV2_3(W(1,3),W(1,4),GC_26,MW, WW, W(1,5))
      CALL OXXXXX(P(0,5),ZERO,NHEL(5),+1*IC(5),W(1,6))
      CALL IXXXXX(P(0,6),ZERO,NHEL(6),-1*IC(6),W(1,7))
      CALL FFV2_3(W(1,7),W(1,6),GC_26,MW, WW, W(1,8))
      CALL VVT2_3_3(W(1,8),W(1,5),GC_68,GC_11,MH, WH, W(1,9))
C     AMPLITUDE(S) FOR DIAGRAM NUMBER 1
      CALL VVT4_0(W(1,1),W(1,2),W(1,9),GC_11,AMP(1))
      JAMP(1)=+2*(-AMP(1))
      ...
      DO I = 1, NCOLOR
        ...
        ENDDO
        GGYWW = GGYWW+ZTEMP*DCONJG(JAMP(I))/DENOM(I)
      ENDDO
      END
