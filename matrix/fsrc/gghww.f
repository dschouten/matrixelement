...
C     
C     PROCESS: G G > H > E+ VE E- VE~ / A Z HIG=1 HIW=1
C     
      IMPLICIT NONE
      INTEGER RESET
C     
...
C     ----------
      ...
      IF (ISHEL(IMIRROR) .EQ. 0 .OR. NTRY(IMIRROR) .LE. MAXTRIES) THEN
        DO I=1,NCOMB
          IF (GOODHEL(I,IMIRROR) .OR. NTRY(IMIRROR).LE.MAXTRIES .AND. 
     $          (I.EQ.ISELECTEDHEL .OR. ISELECTEDHEL.LE.0)) THEN ! dschoute - added conditional on ihelicity
            T=GGHWW(P ,NHEL(1,I),JC(1))
            DO JJ=1,NINCOMING
     ...
     $         ,POL(JJ)))) THEN
            ...
            ANS=ANS+T
            TS(I)=T
            MECOMB(I)=T/DBLE(IDEN) ! dschoute - keep track of helicity combinations
          ENDIF
        ENDDO
        JHEL(IMIRROR) = 1
     ...
     $       /NCOMB)) THEN
              ...
              PRINT *,'ADDED GOOD HELICITY ',I,T,
     +               NHEL(1,I),NHEL(2,I),TS(I)*NCOMB/ANS
            ENDIF
          ENDDO
        ENDIF
 ...
 10     CONTINUE
      ...
      REAL*8 FUNCTION GGHWW(P,NHEL,IC)
C     
C     GENERATED BY MADGRAPH 5 V. 1.4.2, 2012-02-16
C     BY THE MADGRAPH DEVELOPMENT TEAM
C     PLEASE VISIT US AT HTTPS://LAUNCHPAD.NET/MADGRAPH5
...
C     ----------
      ...
      CALL VVS3_3(W(1,1),W(1,2),GC_8,MH, WH, W(1,7))
      CALL FFV2_3(W(1,3),W(1,4),GC_17,MW, WW, W(1,8))
      CALL FFV2_3(W(1,6),W(1,5),GC_17,MW, WW, W(1,9))
C     AMPLITUDE(S) FOR DIAGRAM NUMBER 1
      CALL VVS2_0(W(1,9),W(1,8),W(1,7),GC_37,AMP(1))
      JAMP(1)=+2*(-AMP(1))
      GGHWW = 0.D0
      DO I = 1, NCOLOR
        ZTEMP = (0.D0,0.D0)
        ...
        ENDDO
        GGHWW=GGHWW+ZTEMP*DCONJG(JAMP(I))/DENOM(I)
      ENDDO
